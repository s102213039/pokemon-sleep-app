/**
 * Deep Analysis of Redirection Permutation Failures
 */
      ...
      for (const queryObj of URL_QUERIES) {
        for (const hashObj of URL_HASHES) {
          ...
          if (navResult.finalPage !== expectedFinalPage) {
            const key = `Storage=${storageObj.name} | Query=${queryObj.name} | Device=${widthObj.name}/${uaObj.name}`;
...
console.log('--- Failure Breakdown by Storage / Query / Device Group ---');
const grouped = {};
Object.entries(failureBreakdown).forEach(([k, count]) => {
  const parts = k.split(' | ');
  const storage = parts[0];
  const query = parts[1];
  const groupKey = `${storage} + ${query}`;
  grouped[groupKey] = (grouped[groupKey] || 0) + count;
});

Object.entries(grouped).forEach(([k, count]) => {
  console.log(`  ${k}: ${count} failed test permutations`);
});
