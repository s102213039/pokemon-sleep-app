/**
 * Inspect failures when starting from app/index.html
 */
const fs = require('fs');
    ...
    for (const storageObj of STORAGE_STATES) {
      for (const queryObj of URL_QUERIES) {
        for (const hashObj of URL_HASHES) {
          ...
          if (navResult.finalPage !== expectedPage) {
            const key = `Query=${queryObj.name} | Storage=${storageObj.name} | Final=${navResult.finalPage}`;
...
console.log('Failures from app/index.html breakdown:');
