...
## 交接：2026-09-26 已完成（純帳號免信箱註冊、彈窗事件殘留變數修復與 v=20260926_06）

- **靜態快取標籤**：`v=20260926_06`
- **測試狀態**：162/162 自動化測試全數通過（含 openAuthModal DOM 生命週期與事件綁定測試、帳號 3~20 字元校驗、密碼 6~32 字元/弱密碼攔截）
- **修復與實作**：
  1. **修復彈窗未定義變數報錯（ReferenceError: configToggle is not defined）**：
     - 先前重構時已自彈窗 DOM 徹底移除「自訂伺服器折疊選單」，但在 `bindAuthModalEvents` 尾部仍殘留對已移除 DOM 變數（`configToggle`、`configSave` 等）的引用，導致點擊登入彈窗時報錯中斷。
     - 已徹底清除所有孤立殘留變數引用，並在 `tests/run_tests.js` 補足 `openAuthModal` 完整 DOM 生命週期與事件綁定之自動化回歸測試。
  2. **全面捨棄電子郵件，採用正常純帳號（Username）註冊與登入**：
     - 使用者無需輸入冗長的 Email，僅需輸入自訂帳號（例如：`trainer123`）。
     - 帳號格式防呆限制：長度 3 ~ 20 字元，僅限英文、數字與底線（`_`），嚴禁空格與特殊符號。
     - 密碼安全防呆限制：長度 6 ~ 32 字元，禁止空白字元、禁止單一字元重複（如 `111111`、`aaaaaa`）、禁止極弱常見組合（如 `123456`、`password`）。
     - 底層自動透過安全唯一識別機制對接中央 Supabase Auth，既保持標準密碼雜湊與加密安全性，又給予使用者 100% 純帳號的簡潔體驗。
  2. **極簡免設定登入/註冊介面 (`#cloud-auth-modal`)**：
     - 表單輸入欄位調整為「自訂帳號」與「自訂密碼」。
     - 頂部導航欄、手機端頭像、倉庫同步膠囊全面展示純使用者帳號（無任何多餘域名或後綴）。
     - 密碼顯示/隱藏切換按鈕全面替換為純向量 SVG 圖標，嚴格落實全域零 Emoji 規範。
     - 支援明確友善的繁體中文錯誤提示（如「帳號長度需介於 3 到 20 個字元之間」、「密碼過於簡單，請更換較安全的組合」等）。
  3. **雙向智慧聯集合併演算法 (`mergePokemonBoxes`)**：
     - 首次登入或跨裝置合併時，依據寶可夢 UID、特徵指紋（`name + level + nature + subskills + ingredients`）與更新時間戳記執行去重聯集合併，確保離線與線上任何新增的寶可夢皆不會被互相覆蓋。
  4. **倉庫模組雙向連動 (`js/modules/box.js`)**：
     - 在 `saveUserBox()` 中自動掛接 `CloudSync.pushCloudBox(userBox)`（800ms 防抖推送）。
     - 註冊 `onRemoteUpdate` 與 `onAuthStateChange`，當手機或另一台裝置同步時，自動更新本地 `userBox`、LocalStorage，並即時重繪卡片與更新評測室快選名單。
  5. **規範檢核**：
     - 嚴格遵守零 Emoji 規範（Zero Emoji Policy）。
     - 嚴格遵守單一外框規範（Single Frame Rule）。
