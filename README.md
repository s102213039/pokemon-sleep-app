...
- 🔍 **多語言 & ID 即時搜尋**：支援中文 (CN)、英文 (EN)、日文 (JP) 與全國圖鑑編號（`#0001`、`1`、`01`）即時檢索。
- 🎴 **雙檢視模式**：直觀卡片檢視（Card Grid）與高密度資料表格檢視（Data Table）無縫切換。
- 🏷️ **側邊欄複合篩選**：支援 18 種屬性、3 大專長、樹果種類、可產食材、僅初始食材、僅最終進化與主技能篩選。
...
- ⚡ **主技能數值資料庫**：
  - Lv.1 ~ Lv.8 完整數值一覽
  - **蓄力 (能量填充S)**：0~10 次蓄積動態晶片切換與對照矩陣
  - **幫手加速 (神獸專屬)**：0~5 種類同屬隊友動態互動試算
  - **食材精選S 專屬食材池**：6 家族 7 種寶可夢候選食材矩陣
- 🧩 **副技能與性格指南**：
  - 🧮 **主技能發動機率矩陣**：基於官方底層公式 `(1 + 副技%) × 性格倍率` 精算 24 種組合發動倍率與評級
  - 📋 **副技能階級與數值總覽**：已開放副技能分類與數值
  - 🧬 **性格五維屬性倍率表**：官方最新補正增減效果
- 🎓 **培育與評級指南**：樹果型、食材型、技能型各專長 Ⓢ/Ⓐ/Ⓑ/Ⓒ 評級標準與睡眠天數基線。
- 🥗 **食材產量天梯榜**：橫向視覺天梯座標圖（產量 TOP 1 標籤、AAA/ABB/ABC 配方、跨度線）、19 種食材天梯清單、產量冠軍與極限日產量標籤，並支援「副技能（食材M、幫速M）」與「性格補正（食材▲▲、幫速▲▲）」四重模擬試算。
- 🫐 **樹果與食材基礎能量庫**：樹果能量等級滑桿（Lv.1~70）、島嶼加成（0~85%）、順果 2x 試算與 19 種食材基礎分。

---

### 4. 📦 寶可夢倉庫與智慧 OCR 辨識 (User Box & Smart OCR)
- 📸 **Tesseract.js OCR 截圖辨識**：支援單張與批次上傳/拖曳遊戲截圖，自動辨識寶可夢物種、性格、等級與 5 項副技能。
- 🛡️ **智慧防重複入庫**：以「物種 + 性格 + 5副技能」生成專屬指紋雜湊，自動剔除重複辨識項目。
- 📝 **全功能手動編輯**：支援自訂暱稱、等級調整 (Lv.1~Lv.80)、食材組合配置與即時刪除/修改。

---

### 5. 🔮 評測實驗室與六維量化雷達 (Appraisal Lab & 6D Radar)
- 📊 **六維量化戰力指標**：基於 RaenonX PR 算法評測 **樹果力、食材力、技能力、速度力、自給力、極限爆發力**。
...
本專案內建一套獨立的 **42 項全自動化測試套件**，涵蓋資料庫完整性、公式精確度、OCR 雜湊去重、i18n 雙語翻譯與 UI 元件渲染。
...
### 測試覆蓋階層
- **Tier 1 - Feature Coverage (22 項)**：資料集結構、公式計算、食譜加成、主技能矩陣。
- **Tier 2 - Boundary Cases (16 項)**：空搜尋、大小寫不敏感、多選交集、等級邊界 (Lv.1~80)。
- **Tier 3 - Cross-Feature (1 項)**：多維度複合篩選與動態排序。
- **Tier 4 - Real-World Scenarios (3 項)**：完整操作流程模擬、評測六維雷達、食材精選專屬食材池提示。
...
2. **資料自動更新**：
   - 在頁面右上角「⚙️ 設定」中輸入具有 `workflow` 權限的 GitHub PAT Token，即可一鍵遠端觸發 GitHub Actions 執行最新資料爬取與公告同步。
